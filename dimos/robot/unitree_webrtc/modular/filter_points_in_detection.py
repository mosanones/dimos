...
def project_points_to_camera(
    points_3d: np.ndarray, camera_matrix: np.ndarray, extrinsics: np.ndarray
) -> np.ndarray:
    ...
    print(f"Sample 3D points in base_link (first 5):")
    print(points_3d[:5])
    print(f"Sample 3D points in camera frame (first 5):")
...
def filter_points_in_detections(
    ...
    points_2d, valid_mask = project_points_to_camera(
    ...
    for detection in detection_list:
        ...
        if detection_points.shape[0] > 0:
            detection_pointcloud = PointCloud2.from_array(
                detection_points,
...
def main():
    ...
    # Filter points for each detection
    ...
    for i, (detection, pc) in enumerate(zip(detection_list, filtered_pointclouds)):
        if pc is not None:
            ...
            print(f"Published {pc.to_numpy().shape[0]} points for {class_name}")

    # Also create a combined point cloud with all filtered points
    if valid_pointclouds:
        # Combine all point arrays
        all_points = np.vstack([pc.to_numpy() for pc in valid_pointclouds])
        combined_pointcloud = PointCloud2.from_array(
